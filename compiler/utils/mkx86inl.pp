  ...
  begin
    ...
    if AWithSize then
      case ATyp of
        ...
        'f32':   exit('LOC_MMREGISTER,OS_M128F');
        'f64':   exit('LOC_MMREGISTER,OS_M128D');
...
function GetLocAllocation(const ATyp: string): string;
  begin
    ...
    case ATyp of
      ...
      'f32':  exit('location.register:=cg.getmmregister(current_asmdata.CurrAsmList, OS_M128F);');
      'f64':  exit('location.register:=cg.getmmregister(current_asmdata.CurrAsmList, OS_M128D);');
