...
{$endif cpu64bitalu}
      ...
      private
        { Either a region is allocated for FItems+h2itemid+next, or the table is empty with hmask=0, fake 'h2itemid' pointing to a single -1, and distinctive FItems=nil. }
        FItems : Psuperregister;
        h2itemid : PInt32; { Count of 'h2itemid's is always hmask + 1 and is always a power of two. h2itemid[h] = -1 means empty, h2itemid[h] = itemid >= 0 references FItems[itemid]. }
        ...
        class function allocateregion(
          nh,amaxitems:uint32; out aitems:Psuperregister; out ah2itemid:PInt32; out anext:PInt32):SizeUint; static;
        class procedure rebuildh(aitems:Psuperregister; ah2itemid,anext:PInt32; ahmask,anitems:int32); static;
      public
...
******************************************************************************}
    ...
    const
      SuperRegisterWorkHashList_EmptyH2itemid: int32 = -1;


    constructor Tsuperregisterworkhashlist.init;
    begin
      h2itemid:=@SuperRegisterWorkHashList_EmptyH2itemid;
    end;
    ...
    begin
      if x.FNItems=0 then
        ...
        begin
          datasize:=allocateregion(1+x.hmask,x.maxItems,FItems,h2itemid,next);
    ...
    begin
      if (i<0) or (i>=length) then
        internalerror(2022050302);
      { Remove #i reference from h2itemid/next. }
      nextp:=h2itemid+FItems[i] and hmask;
      repeat
        ii:=nextp^;
        if ii=i then
          break;
      ...
      { Move item #length-1 = #ilast to #i and fix up its reference in h2itemid/next. }
      ilast:=length-1;
      if i<>ilast then
        begin
          nextp:=h2itemid+FItems[ilast] and hmask;
          repeat
            ii:=nextp^;
            if ii=ilast then
              break;
    ...
    begin
      ...
      repeat
        ...
        p:=p shr 1;
      until p=0;
      rebuildh(FItems,h2itemid,next,hmask,FNItems);
    end;
    ...
    begin
      ...
      allocateregion(1+newHMask,newMaxItems,newItems,newH2itemid,newNext);
      ...
      if hmask=newHMask then
        begin
          ...
          Move(h2itemid^,newH2itemid^,(1+newHMask)*sizeof(h2itemid^));
          Move(next^,newNext^,FNItems*sizeof(next^));
        end
      else
        rebuildh(newItems,newH2itemid,newNext,newHMask,FNItems);

      if Assigned(FItems) then
        FreeMem(FItems);
      FItems:=newItems;
      h2itemid:=newH2itemid;
      next:=newNext;
      hmask:=newHMask;
    ...
    class function Tsuperregisterworkhashlist.allocateregion(
      nh,amaxitems:uint32; out aitems:Psuperregister; out ah2itemid:PInt32; out anext:PInt32):SizeUint;
    var
      h2itemidOffset,nextOffset,ofs: SizeUint;
    begin
      ofs:=align(sizeof(Tsuperregister)*amaxitems,sizeof(int32)); { items + align to h2itemid }
      h2itemidOffset:=ofs;
      ofs:=align(ofs+nh*sizeof(int32),sizeof(int32)); { + h2itemid + align to next }
      nextOffset:=ofs;
      result:=ofs+amaxitems*sizeof(int32); { + next }
      aitems:=GetMem(result);
      ah2itemid:=pointer(aitems)+h2itemidOffset;
      anext:=pointer(aitems)+nextOffset;
    end;


    class procedure Tsuperregisterworkhashlist.rebuildh(aitems:Psuperregister; ah2itemid,anext:PInt32; ahmask,anitems:int32);
    var
      ii : SizeInt;
      hp : PInt32;
    begin
      FillDWord(ah2itemid^,1+ahmask,dword(-1));
      FillDWord(anext^,anitems,dword(-1));
      for ii:=0 to anitems-1 do
        begin
          hp:=ah2itemid+aitems[ii] and ahmask;
          anext[ii]:=hp^;
          hp^:=ii;
        end;
    end;
