        ...
        if taicpu(p).oper[1]^.typ = top_reg then
          begin
            ...
            if GetNextInstruction_p and (hp1.typ = ait_instruction) then
              begin
                { Only do the register simplificaiton optimisation on -O3 and
                  above so as to aid debugging.  -O1 tends to be less efficient
                  with register allocations anyway, such as not utilising
                  regvars, and for -O2 it's just too slow }
                if (cs_opt_level3 in current_settings.optimizerswitches) then
                  repeat
                  ...
                  A_MOV:
                    begin
                      if (cs_opt_level3 in current_settings.optimizerswitches) and
                        ...
                        (taicpu(hp2).oper[0]^.typ in [top_reg, top_ref]) and
                        { We're using NewMMReg as a placeholder }
                        DoMOVRegisterSimplification(hp2, hp_deleted, NewMMReg) then
                        begin
                          Include(OptsToCheck, aoc_ForceNewIteration);
                              ...
                              begin
                                ...
                                if ReplaceResult then
                                  begin
                                    ...
                                    hp2 := tai(hp2.Next);
                                    if not Assigned(hp2) then
                                      { We should hit at least hp1 first }
