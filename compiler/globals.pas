...
{$if defined(xtensa) or defined(riscv32) or defined(arm)}
     function DefaultReplacements_GetIDFVersion(param: pointer): ansistring;
       begin
         Result:=idfversionstring(idf_version);
       end;
{$endif xtensa/riscv32/arm}
...
{$endif}
{$if defined(xtensa) or defined(riscv32) or defined(arm)}
                or (idf_version > 0) and TryReplaceAtSp('$IDF_VERSION',@DefaultReplacements_GetIDFVersion)
                or (idfpath <> '') and TryReplaceAtSp('$IDFPATH',idfpath)
{$endif xtensa/riscv32/arm}
               then
                 continue;
