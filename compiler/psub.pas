...
{$endif defined(arm) or defined(m68k)}
    ...
    const
      ...
      NOPURE_GLOBAL_VARIABLE = 0;
      NOPURE_WRITABLE_CONST = 1;
      NOPURE_IMPURE_CALL = 2;
      NOPURE_POINTER_DEREF = 3;
      NOPURE_ADDRESSOF = 4;
        ...
        case n.nodetype of
          loadn:
            if (TLoadNode(n).symtableentry.typ=staticvarsym) then
              begin
                { Writable constants are a bit difficult to handle at the moment,
                  so disable support for the time being }
(*                if (
                    { Take no chances }
                  ...
                  begin
                    ...
                    PInteger(arg)^:=NOPURE_WRITABLE_CONST;
                  end
          ...
          calln:
            if not(Assigned(TCallNode(n).procdefinition)) or
              ...
              begin
                result:=fen_norecurse_true;
                PInteger(arg)^:=NOPURE_IMPURE_CALL;
              end;
          derefn:
            begin
              { Not allowed to dereference a pointer }
              result:=fen_norecurse_true;
              PInteger(arg)^:=NOPURE_POINTER_DEREF;
            end;
          addrn:
            begin
              { Not allowed to take the address of a variable }
              result:=fen_norecurse_true;
              PInteger(arg)^:=NOPURE_ADDRESSOF;
            end;
          else
            ;
        end;
    ...
    function checknodepurity(procdef: tprocdef; coderoot: TNode): boolean;
      ...
      procedure _no_pure(const reason: TMsgStr);
        begin
          Message2(parser_e_pure_ineligible,tprocdef(procdef).procsym.realname,reason);
        end;
