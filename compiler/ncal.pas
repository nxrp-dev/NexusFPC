...
interface
    ...
    type
       ...
       protected
          ...
          class function has_unconditional_break(n: tstatementnode): tnode; static;
    ...
    procedure tcallparanode.firstcallparan;
      begin
        ...
        { for targets that have to copy "value parameters by reference" on the
          ...
          callnode may not be assigned in case firstcallparan is called for
      ...
      begin
        ...
        if (po_pure in procdefinition.procoptions) and
          ...
          begin
            ...
            { Check if we can make the call pure when it references procs/vars
...
**************************************************************************}
    ...
    class function tcallnode.has_unconditional_break(n: tstatementnode): tnode;
      begin
        result:=nil;
        while Assigned(n) do
          begin
            case n.statement.nodetype of
              blockn:
                begin
                  ...
                  result:=has_unconditional_break(TStatementNode(TBlockNode(n.statement).statements));
                  if Assigned(result) then
                    Exit;
                end;
              ...
              statementn:
                begin
                  ...
                  result:=has_unconditional_break(TStatementNode(TStatementNode(n.statement).statement));
                  if Assigned(result) then
                    Exit;
                end;
            ...
            begin
              ...
              if
                ...
                begin
                  ...
                  MessagePos2(n.fileinfo,parser_e_pure_ineligible,tprocdef(procdefinition).procsym.realname,'non-deterministic for loop');
    ...
    function tcallnode.pass1_pure:tnode;
      var
        ...
        { Use this to keep track of parameters that had their var parameters
          temporarily changed to out parameters for the sake of data-flow analysis}
        ChangedVarParams: TFPHashObjectList;

        function is_valid_assignment(InNode: TNode; out OutNode: TNode): Boolean;
          begin
            result:=false;
            if (InNode.nodetype<>assignn) then { Statement is not an assignment }
              Exit;

            if not is_constnode(tassignmentnode(InNode).right) then { Not writing a constant? }
              Exit;

            case tassignmentnode(InNode).left.nodetype of
              loadn:
                begin
                  OutNode:=tassignmentnode(InNode).left;
                  Exit(true);
                end;

              typeconvn:
                if (ttypeconvnode(tassignmentnode(InNode).left).convtype=tc_equal) and
                  (ttypeconvnode(tassignmentnode(InNode).left).left.nodetype=loadn) and
                  (
                    { Just check that the types are the same }
                    ttypeconvnode(tassignmentnode(InNode).left).totypedef=
                    ttypeconvnode(tassignmentnode(InNode).left).left.resultdef
                  ) then
                  begin
                    OutNode:=ttypeconvnode(tassignmentnode(InNode).left).left;
                    Exit(true);
                  end;
              else
                { Assignment not loading a symbol }
                Exit;
            end;
          end;

        procedure pure_cleanup;
          var
            X: Integer;
          begin
            { Those parameters we changed from var to out, change back to var }
            for X:=0 to ChangedVarParams.Count-1 do
              TParaVarSym(ChangedVarParams[X]).varspez:=vs_var;

            ChangedVarParams.Free;
        ...
        procedure not_all_outputs;
          begin
            MessagePos2(fileinfo,parser_e_pure_ineligible,tprocdef(procdefinition).procsym.realname,'not all outputs were assigned a value');
            Exclude(procdefinition.procoptions, po_pure);
{$ifdef DEBUG_PURE}
...
{$endif DEBUG_PURE}
        ...
        procedure non_trivial;
          begin
            MessagePos2(fileinfo,parser_e_pure_ineligible,tprocdef(procdefinition).procsym.realname,'analysis did not produce a simple assignment to the outputs');
            Exclude(procdefinition.procoptions, po_pure);
{$ifdef DEBUG_PURE}
...
{$endif DEBUG_PURE}
        ...
        function simplify_block: boolean;
          var
...
{$endif DEBUG_PURE}
      ...
      begin
        ...
        ChangedVarParams:=TFPHashObjectList.Create(False);
              ...
              vs_value,vs_const:
                begin
                  ...
                  if not is_constnode(para.left) then
                    begin
                      current_procinfo := nil; { Prevent pure_cleanup from freeing it }
                      pure_cleanup;
{$ifdef DEBUG_PURE}
                      WriteLn('-- ABORT PURITY ANALYSIS OF ', TProcDef(procdefinition).fulltypename, ' --');
...
{$endif DEBUG_PURE}
              ...
              vs_var:
                begin
                  ...
                  ChangedVarParams.Add(hexstr(para.parasym), para.parasym);
...
{$endif DEBUG_PURE}
          ...
          if changed or (run=1) then { Always run constant propagation on the first run }
            repeat
              ...
              { Changed gets overwritten by simplify_block, but even if it was set
...
{$endif DEBUG_PURE}
        ...
        if (outputs.Count=0) then
          begin
            ...
            if not Assigned(purestatements) then
              begin
                { Empty procedure or result wasn't written to }
                if not assigned(funcretnode) then
                  Result:=cnothingnode.create()
                else
                  not_all_outputs;
            ...
            if (
              ...
              not is_valid_assignment(purestatements.statement,symbolload) then
              begin
                non_trivial;
                pure_cleanup;
                Exit;
          ...
          begin
            ...
            while Assigned(purestatements) do
              begin
                ...
                if not is_valid_assignment(purestatements.statement,symbolload) then
                  begin
                    non_trivial;
                    outputbody.Free;
                    pure_cleanup;
                ...
                if (tloadnode(symbolload).symtableentry.typ in [localvarsym, paravarsym]) and
                  (vo_is_funcret in tabstractnormalvarsym(tloadnode(symbolload).symtableentry).varoptions) then
                  begin
(*                    maybe_create_funcret_node;
...
*)
                ...
                if (tloadnode(symbolload).symtableentry.typ=paravarsym) then
                  begin
                    ...
                    for X:=Outputs.Count-1 downto 0 do
                      begin
                        n:=tnode(Outputs[X]);
                        if (tloadnode(symbolload).symtableentry=tcallparanode(n).parasym) then
                          begin
                    ...
                    if not (vo_is_funcret in tabstractnormalvarsym(tloadnode(symbolload).symtableentry).varoptions) then
                      begin
                        addstatement(
                          outputstatements,
                          cassignmentnode.create(
